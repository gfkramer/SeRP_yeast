...
(Asite_offset_demo.txt) must be provided defining the offset for each footprint length.
...
def getReferenceFiles(path_current):
    ...
    dictReads = pickle.load(open(path_ref + 'yeast_sequence_demo.pkl', 'rb'))
    dictGenes = pickle.load(open(path_ref + 'yeast_transcripts_demo.pkl', 'rb'))
    dicttRNA = pickle.load(open(path_ref + 'yeast_tRNA_demo.pkl', 'rb'))
...
def getOffset(path_current):
    ...
    path_ref = os.path.join(path_current, 'references_yeast_demo', '')
    dictOffset = {}
    with open(path_ref + 'Asite_offset_demo.txt', 'r') as f:
        for line in f: 

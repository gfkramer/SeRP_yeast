...
def BindingDetection(input_path, file_total1, file_total2, file_selec1, file_selec2, output_name): 
    ...
    # read in gene expression files to exclude genes that have less than reads 
    # than given threshold from the downstream analysis; write all genes that 
    # are included in both samples into a list:
...
'''
>> to run this script via IDLE or another environment replace lines 898-934 by 
